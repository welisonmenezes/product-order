...
$(window).on('load', function() {
    ...
    selecEstado.on('change', function() {
        var estadoNome = $(this).val();
        selectCidade.html('');
        ...
        if (estadoNome) {
            ...
            if (estadoSelecionado && estadoSelecionado[0] && estadoSelecionado[0].ID) {
                var cidadesByEstado = cidades.filter(function(cidade) {
                    return cidade.Estado == estadoSelecionado[0].ID;
                });
                if (cidadesByEstado && cidadesByEstado.length) {
                    cidadesByEstado.map(function(cidade) {
                        selectCidade.append($('<option>', { 
                            value: cidade.Nome,
                            text : cidade.Nome
                        }));
